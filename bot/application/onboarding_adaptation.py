...
- Мы не используем менторов, наставников, кураторов, преподавателей, экспертов, консультации, созвоны, чаты с людьми или проверку человеком.
- Не обещай помощь человека, сопровождение, личную проверку, разбор работ или обратную связь от специалиста.
- Маршрут состоит только из самостоятельных материалов: статьи, видео, курсы, задания, чек-листы и самопроверка по критериям.
- Если во входных данных встречается старый код mentor или текст про обратную связь, трактуй это как самопроверку по понятным критериям.
- Не используй англицизмы вроде roadmap, feedback, mentor, skills, track, sprint, Mini App; пиши «маршрут», «самопроверка», «навыки», «направление», «этап», «приложение».

Формат строго JSON:
{"title":"до 70","subtitle":"до 160","manual_hint":"до 100","support":"до 32","option_labels":["по числу base_options, до 32"]}

Пиши значения JSON строго русскими словами, коротко и дружелюбно. Исключение: названия профессий из входных данных можно оставить как есть. Ключи JSON оставь как в схеме. Сохраняй смысл каждой кнопки.
""".strip()
...
highlights: ровно 3 строки. Пиши значения JSON строго русскими словами, тепло и без воды. Исключение: названия профессий из входных данных можно оставить как есть. Ключи JSON оставь как в схеме.
""".strip()
...
DISALLOWED_GENERATED_RE = re.compile(
    r"(https?://|www\.|callback_data|authorization|bearer|system prompt|developer message|ignore previous|игнорируй предыдущ|системн\w+ промпт|/start|/restart|/debug|roadmap|feedback|mentor|skills|track|sprint|mini app|ментор|наставник|куратор|преподавател|эксперт|консультац|созвон|чат с|провер[а-я]* человеком|обратн[а-я]* связ)",
    re.IGNORECASE,
)
