...
В репозитории подготовлен прототип Telegram-бота и React Mini App для конструктора персональных маршрутов обучения. Бот является основной точкой входа: собирает профиль пользователя, строит краткий маршрут, принимает обратную связь и открывает Mini App кнопкой `web_app`.
